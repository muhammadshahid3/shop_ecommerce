    ...
    @foreach($products as $key => $product)
      ...
      <div class="col-xl-4 col-md-6 col-sm-6 infinite-item">
        <div class="tp-product-item-2 mb-40">
          ...
          <span><span><span>
            <input type="hidden" name="{{$product->product_slug}}_c_slug" value="{{$product->category_slug}}">
            <input type="hidden" name="{{$product->product_slug}}_name" value="{{$product->product_name}}">
            <input type="hidden" name="{{$product->product_slug}}_price" value="{{GetTwodecimalHelper($product->product_price)}}">
            <input type="hidden" name="{{$product->product_slug}}_s_qty" value="{{$product->stock_quantity}}">
            <input type="hidden" name="{{$product->product_slug}}_thumbnail" value="{{$product->product_thumbnail}}">
          </span></span></span> 
             
          <div class="tp-product-thumb-2 p-relative z-index-1 fix w-img">
              <a href="{{route('product.detail', [$product->product_slug, FilterStringHelper($product->product_name)])}}">
                <img src="{{asset('upload/product/').'/'.$product->product_thumbnail}}" alt="no pic">
              </a>
  
              <!-- product action -->
              <div class="tp-product-action-2 tp-product-action-blackStyle">
                <div class="tp-product-action-item-2 d-flex flex-column">
                    <button type="button" class="tp-product-action-btn-2 tp-product-add-cart-btn" onclick="addProductBasket({{$product->product_slug}})">
                    ...
                    <button type="button" class="tp-product-action-btn-2 tp-product-add-to-wishlist-btn" onclick="addProductWishlist({{$product->product_slug}})">
          ...
          <div class="tp-product-content-2 pt-15">
              ...
              <div class="tp-product-price-wrapper-2">
                <span class="tp-product-price-2 new-price">${{GetTwodecimalHelper($product->product_old_price)}}</span>
                <span class="tp-product-price-2 old-price">${{GetTwodecimalHelper($product->product_price)}}</span>
              </div>
          </div>
        </div>
      </div>
