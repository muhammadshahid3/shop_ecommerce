...
@section('content')
    ...
    <div class="container mt-4 mb-4">
        ...
        @foreach ($search_product as $search_product_list)
               ...
               <div class="tp-product-item p-relative transition-3 mb-25">
                  <div class="tp-product-thumb p-relative fix m-img thumb-max-height">
                     <a href="${product_element_product_route}">
                        <img src="{{asset('upload/product/'.$search_product_list->product_thumbnail)}}" alt="product-pic">
                     </a>
                     <!-- product badge -->
          ...
          <!-- <span><span><span>
            <input type="hidden" name="${product_element.product_slug}_c_slug" value="${product_element.product_slug}">
            <input type="hidden" name="${product_element.product_slug}_name" value="${product_element.product_name}">
            <input type="hidden" name="${product_element.product_slug}_price" value="${product_element.product_price}">
            ...
            <!-- Product_hidden fields End -->
                  ...
                  <div class="tp-product-content">
                     <div class="tp-product-category">
                        <a href="">  {{$search_product_list->category_name}}</a>
                     </div>
                     <h3 class="tp-product-title title-min-height">
                        <a href="">
                            {{$search_product_list->product_name}}
                        </a>
                     </h3>
                     ...
                     <div class="tp-product-price-wrapper">
                        <span class="tp-product-price old-price">${{ number_format($search_product_list->product_old_price, 2)}}</span>
                        <span class="tp-product-price new-price">${{ number_format($search_product_list->product_price,2)}}</span>
                     </div>
                  </div>
               </div>
            </div>
